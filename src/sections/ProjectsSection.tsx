import { projects } from '../data/projects'
import { OSStatus } from '../components/OSStatus'
import { Projects } from '../components/Projects'
import { founderOS } from '../core/founder-os'

export function ProjectsSection() {
  return (
    <>
      {/* OS Snapshot — part of the Founder OS system, not a standalone section */}
      <section id="os-status" className="section-divider py-20 sm:py-28">
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="eyebrow mb-4">02 — OS Snapshot</div>
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-10">
            <h2 className="text-[clamp(1.8rem,4vw,3rem)] font-semibold tracking-tight leading-[1.1] text-text">
              System State
            </h2>
            <a href="#founder-os" className="font-mono text-[11px] text-dim hover:text-cyan transition-colors self-start sm:self-auto">
              ← Part of Founder OS
            </a>
          </div>
          <OSStatus snapshot={founderOS.snapshot} />
        </div>
      </section>

      {/* Case Studies — output system */}
      <section id="projects" className="section-divider py-24 sm:py-32" style={{ background: '#0a0a14' }}>
        <div className="max-w-7xl mx-auto px-5 sm:px-8">
          <div className="eyebrow mb-4">03 — Case Studies</div>
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-12">
            <h2 className="text-[clamp(1.8rem,4vw,3rem)] font-semibold tracking-tight leading-[1.1] text-text">
              Output Systems.
              <br />
              <span className="text-gradient-cyan">Engineering analysis</span> per build.
            </h2>
            <a href="https://github.com/legacyasphere-id" target="_blank" rel="noopener"
              className="inline-flex items-center gap-2 text-[13px] text-dim hover:text-cyan transition-colors self-start lg:self-auto">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0 1 12 6.844a9.59 9.59 0 0 1 2.504.337c1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.02 10.02 0 0 0 22 12.017C22 6.484 17.522 2 12 2z"/></svg>
              View all on GitHub →
            </a>
          </div>
          <Projects projects={projects} />
        </div>
      </section>
    </>
  )
}
